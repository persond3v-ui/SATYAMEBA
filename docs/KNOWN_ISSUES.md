...
- **N2/N3 🔴 Open** — deleting a user removes the DB row but **not** the per-user
  volume / Hub state (keyed by username), so a re-registered same username could
  inherit old files. Fix: namespace storage by immutable user-id, or a Hub-side
  volume-delete hook. (Audited; manual reclaim in the meantime.)
